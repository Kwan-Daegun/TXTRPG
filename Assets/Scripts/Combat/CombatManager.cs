    ...
    private void HandleEnemyDeath(EnemyRuntimeData enemy)
{
    ...
    {
        ulong requesterId = NetworkManager.Singleton != null &&
    ...
    {
        RunAttackServer(rpcParams.Receive.SenderClientId);
    }
    ...
    private void PlayerAttack(PlayerRunTimeData attacker)
    {
    ...
    private void EnemyTurnPhase()
    {
        ...
        {
            if (!GameManager.Instance.IsPartyAlive()) break;
            ...
            else if (target.IsDodge())
            {
                ...
                result.damage = 0;
                result.message =
                    $"{target.playerName} dodged {enemy.enemyName}'s attack!";
            }
            else if (target.IsBlock())
            {
                ...
                result.damage = blocked;
                result.message =
                    $"{target.playerName} blocked! Only {blocked} damage taken.";
            }
            else
            {
                target.TakeDamage(rawDamage);
                ...
                if (target.IsCounter())
                {
                    ...
                    LogResult(new CombatResult
                    {
                        ...
                        damage = rawDamage,
                        message =
                            $"{target.playerName} COUNTERS for {rawDamage} damage!"
                    });
                }
            }
